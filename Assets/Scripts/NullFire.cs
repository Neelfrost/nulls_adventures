...
public class NullFire : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        else if (other.CompareTag("Enemy")) // 50% chance to one shot Enemy
        {
            _system.Stop();

            if (Random.value >= 0.5)
                other.GetComponent<Enemy>().Damage(10.0f);
        }
    }
    ...
    public void Shoot(int direction)
    {
        ...
        _projectile.AddForce(new Vector2(speed * Mathf.Sign(direction), 0.0f), ForceMode2D.Impulse);
    }
